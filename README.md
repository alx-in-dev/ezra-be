...
заражение территории, PvP/PvE). Только Go-сервер, клиент (Unity) живёт в
основном монорепозитории.

> Этот репозиторий — зеркало `server/` из [ezra](https://github.com/alx-in-dev/ezra)
> (источник правды), нужен для скоуп-доступа контрибьюторам бэкенда. PR сюда
> в итоге переносятся обратно в монорепо.
